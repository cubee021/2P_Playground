...
AMyCharacterBase::AMyCharacterBase()
{
	...
	static ConstructorHelpers::FObjectFinder<UMyComboAttackData> ComboAttackDataRef(TEXT("/Script/Project2.MyComboAttackData'/Game/Project2/CharacterAction/DA_ComboAttack.DA_ComboAttack'"));
...
void AMyCharacterBase::ProcessComboCommand()
{
	if (CurrentCombo == 0)
	{
		...
		{
			ComboAttackBegin(ComboPunchMontage);
		}
		else if(GetCurrentWeaponType() == EWeaponType::Melee)
		{
			ComboAttackBegin(ComboMeleeMontage);
		}
...
void AMyCharacterBase::ComboAttackBegin(UAnimMontage* InMontage)
{
	CurrentCombo = 1;
	...
	AnimInstance->Montage_Play(InMontage, AttackSpeedRate);
	...
	if (GetCurrentWeaponType() == EWeaponType::NoWeapon)
	{
		SetComboPunchCheckTimer();
	}
	else if (GetCurrentWeaponType() == EWeaponType::Melee)
	{
		SetComboMeleeCheckTimer();
	}
}
...
void AMyCharacterBase::SetComboPunchCheckTimer()
{
	...
	{
		GetWorld()->GetTimerManager().SetTimer(ComboTimerHandle, this, &AMyCharacterBase::ComboPunchCheck, ComboEffectiveTime, false);
	}
}

void AMyCharacterBase::SetComboMeleeCheckTimer()
{
	int32 ComboIndex = CurrentCombo - 1;
	ensure(ComboMeleeData->EffectiveFrameCount.IsValidIndex(ComboIndex));

	const float AttackSpeedRate = Stat->GetTotalStat().AttackSpeed;
	float ComboEffectiveTime = (ComboMeleeData->EffectiveFrameCount[ComboIndex] / ComboMeleeData->FrameRate) / AttackSpeedRate;
	if (ComboEffectiveTime > 0.f)
	{
		GetWorld()->GetTimerManager().SetTimer(ComboTimerHandle, this, &AMyCharacterBase::ComboMeleeCheck, ComboEffectiveTime, false);
	}
}

void AMyCharacterBase::ComboPunchCheck()
{
	ComboTimerHandle.Invalidate();
	if (HasNextComboCommand)
	{
		...
		AnimInstance->Montage_JumpToSection(NextSection, ComboPunchMontage);
		SetComboPunchCheckTimer();
		HasNextComboCommand = false;
	}
}

void AMyCharacterBase::ComboMeleeCheck()
{
	ComboTimerHandle.Invalidate();
	if (HasNextComboCommand)
	{
		...
		CurrentCombo = FMath::Clamp(CurrentCombo + 1, 1, ComboMeleeData->MaxComboCount);
		FName NextSection = *FString::Printf(TEXT("%s%d"), *ComboMeleeData->MontageSectionNamePrefix, CurrentCombo);
		AnimInstance->Montage_JumpToSection(NextSection, ComboMeleeMontage);
		SetComboMeleeCheckTimer();
		HasNextComboCommand = false;
	}
}
