...
void AMyCharacterPlayer::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	EnhancedInputComponent->BindAction(DropItemAction, ETriggerEvent::Triggered, this, &AMyCharacterPlayer::ToggleItem);
}
		...
		if (ProjectileClass)
		{
			...
			if (World)
			{
				...
				if (Projectile)
				{
					...
					Projectile->FireInDirection(LaunchDirection, Stat->GetTotalStat().Attack);
...
void AMyCharacterPlayer::ToggleItem()
{
	if (ToggleWeapon == 0)
	{
		...
		if (HasAuthority())
		{
			MulticastRPCToggleWeapon(this, ToggleWeapon);
		}
		else
		{
			ServerRPCToggleWeapon(this, ToggleWeapon);
		}
...
void AMyCharacterPlayer::ServerRPCToggleWeapon_Implementation(AActor* Actor, int32 ToggleIndex)
{
	AMyCharacterPlayer* Character = Cast<AMyCharacterPlayer>(Actor);
	if (Character)
	{
		if (ToggleIndex == 0)
		{
			Character->TakeItem(NoWeaponData);
		}
		else
		{
			Character->TakeItem(SavedItemData);
		}
		
	}
}

void AMyCharacterPlayer::MulticastRPCToggleWeapon_Implementation(AActor* Actor, int32 ToggleIndex)
{
	AMyCharacterPlayer* Character = Cast<AMyCharacterPlayer>(Actor);
	if (Character)
	{
